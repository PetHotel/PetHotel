...
public class FriendsDaoImpl implements IFriendsDao {
	...
	@Override
	public int insert(Friends friend) {
		...
		int use_uid = friend.getUse_uid();
		// 步骤4：设置添加SQL语句模板
		String strSQL = "insert into friends values(null,?,?)";
		Object[] params = new Object[] { uid, use_uid };
	...
	public List<Friends> selectAll() {
		...
		// 步骤3：创建查询语句的模板
		String strSQL = "select * from Friends order by fsid";
		...
		try {
			while (resultSet.next()) {
				...
				friend.setFsid(resultSet.getInt(1));
				friend.setUid(resultSet.getInt(2));
				friend.setUse_uid(resultSet.getInt(3));
	...
	@Override
	public int deleteById(int fsid) {
		// TODO Auto-generated method stub
		// 步骤1：获取一个数据库连接对象
		this.conn = this.connectionManager.openConnection();
		// 步骤2：开启事务
		...
		TransactionManager.beginTransaction();
		// 步骤3：创建SQL语句模板
		String strSQL = "delete from friends where fsid=?";
		Object[] params = new Object[] { fsid };
	...
	public Friends selectById(int fsid) {
		// TODO Auto-generated method stub
		// 步骤1：获取一个数据库连接对象
		this.conn = this.connectionManager.openConnection();
		// 步骤2：创建SQL语句模板
		String strSQL = "select * from friends where fsid=?";
		Object[] params = new Object[] { fsid };
	...
	public int update(Friends friend) {
		...
		TransactionManager.beginTransaction();
		// 步骤3：创建SQL语句模板
		String strSQL = "update friends set uid=?, use_uid=? where fsid=?";
		Object[] params = new Object[] { friend.getUid(), friend.getUse_uid(), friend.getFsid() };
		// 步骤4：调用dbutils中的方法完成对数据库的删除操作
		int affectedRwos = this.dbUtils.execOthers(conn, strSQL, params);
		// 步骤5：根据步骤4的操作结果提交或回滚事务
		if (affectedRwos > 0) {
	...
	@Override
	public List<Friends> selectByUid(int uid) {
		// TODO Auto-generated method stub
		List<Friends> lstFriends = new ArrayList<Friends>();
		// 步骤2：获取一个数据库的连接对象
		this.conn = connectionManager.openConnection();
		// 步骤3：创建查询语句的模板
		String strSQL = "select * from Friends where uid=? order by fsid";
		// 步骤4：使用dbutils方法实现查询操作
	...
	@Override
	public Friends selectByObject(int uid, int use_uid) {
		// TODO Auto-generated method stub
		// 步骤1：获取一个数据库连接对象
				this.conn = this.connectionManager.openConnection();
				// 步骤2：创建SQL语句模板
				String strSQL = "select * from friends where uid=? and use_uid=?";
				Object[] params = new Object[] { uid, use_uid };
				// 步骤4：调用dbutils中的方法完成对数据库的查询操作
				ResultSet resultSet = this.dbUtils.execQuery(conn, strSQL, params);
				// 步骤5：获取结果集合并封装成一个对象
