	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isMultipart){
			...
			try {
				...
				for (FileItem fileItem : fileItems) {
					...
					if(fileItem.isFormField()){
						...
						if("uid".equalsIgnoreCase(name)){
							...
							pets.setUid(Integer.parseInt(value));
							System.out.println("主人：> " + value);
						}	
				...
				boolean flag = petsBiz.add(pets);
				
				
				if(flag){
					String url = "/PetHotel/FindAllPetsByUidServlet?choice=0";
					response.sendRedirect(url);
				}else{
					String error="输入内容错误，添加宠物失败，请后退重新添加并确认信息完整";
...
//				String url = flag?"/PetHotel/FindAllPetsByUidServlet?choice=0":"jsp/Error.jsp";
//				response.sendRedirect(url);
