...
public class AddSenderPostsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddSenderPostsServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMultipart){
		   ...
		   ServletFileUpload sfu = new ServletFileUpload(factory);
			try {
				...
				posts.setRetyeId(-Integer.parseInt(pid));
				
				IPetsBiz petsBiz=new PetsBizImpl();
				Pets pets=petsBiz.findById(Integer.parseInt(pid));
				pettype=pets.getPtype()+"|"+pets.getPvariety();
				...
				if (flag) {
					...
					boolean uflag = userBiz.updateReputation(user.getUid(), user.getReputation()+4);
					if(uflag){
		...
		if(!erro.isEmpty()){
			...
			response.sendRedirect("jsp/showPosts.jsp?pos=0&potype=publicSenderPosts");
		}
