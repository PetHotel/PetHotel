...
/**
 * Servlet implementation class AddSenderPostsServlet
 */
public class AddReceivePostsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddReceivePostsServlet() {
        super();
		   ...
		   ServletFileUpload sfu = new ServletFileUpload(factory);
			try {
				// ★★特别注意★★ 在循环遍历表单数据之前创建实体类对象
				 ...
				 String pobegintime="",poendtime="";
				 String images="";
				 String pettype="",subpettype="";
				 String pocontext="",potype="",pid="";
				 String pomark="";
				...
				for (FileItem fileItem : fileItems) {
					...
					if (fileItem.isFormField()) {
						...
						if ("bdate".equalsIgnoreCase(name)) {
							String value = fileItem.getString();
							value = new String(value.getBytes("iso8859-1"),
									"UTF-8");
							pobegintime=value;
						...
						if ("edate".equalsIgnoreCase(name)) {
							String value = fileItem.getString();
							value = new String(value.getBytes("iso8859-1"),
									"UTF-8");
							poendtime=value;
						...
						if ("pettype".equalsIgnoreCase(name)) {
							String value = fileItem.getString();
							value = new String(value.getBytes("iso8859-1"),
									"UTF-8");
							pettype=value;
						...
						if ("subpettype".equalsIgnoreCase(name)) {
							String value = fileItem.getString();
							value = new String(value.getBytes("iso8859-1"),
									"UTF-8");
							subpettype=value;
				...
				posts.setUid(user.getUid());         //current cus.....
				posts.setRetyeId(-1);
			
				posts.setPettype(pettype+"|"+subpettype);
				if(!images.isEmpty()){
				...
				if (flag) {
					...
					boolean uflag = userBiz.updateReputation(user.getUid(), user.getReputation()+6);
					if(uflag){
						user.setReputation(user.getReputation()+6);
...
//			String error="输入内容错误，添加宠物失败";
			...
			response.sendRedirect("jsp/showPosts.jsp?pos=0&potype=publicReceivePosts");
		}
