...
<body>
  <c:if  test="${empty requestScope.lstPets }">
  	 <script> 
  	  alert("添加宠物信息才可发求收养贴") ;
...
<!-- ******************* Write Here ************************************-->					
	...
	<form action="/PetHotel/AddSenderPostsServlet" id="addSenderPostsForm"  name="addSenderPostsForm"  method="post" enctype="multipart/form-data" >
						 <br />
						 <div id="outer">
			 	   			  <div id="tilte" align="center">
									<p ><strong><font size="5px">求收养帖子填写</font></strong>
				                     </p>
			                  </div><!-- end tilte-->
				  
				   
							  <div>
							   <label>选择想寄养的宠物</label>
							  <select name="pid" id="pid">
						             <c:forEach items="${requestScope.lstPets }" var="pets">
									<option value="${pets.pid}">${pets.pnickname}</option>
									</c:forEach>
							  </select>
						   </div>
...
报酬：100元左右【或价格再议】
求领养原因：需要出差5到6天，家里没人照看小狗【宠物】。
宠物习性：爱乱跑，不喜欢吃米饭。
附件说明【可有可无，最好压缩加密】：附件有本人的简历及宠物的健康证明，文件已被加密，想暂时收养爱宠的朋友请在本贴下面点击申请与我取得联系。                                            
								   </textarea>
							   </div>
				          </div><!-- main -->
				  
						  <div id="upload">
						  宠物近期图片上传<input type="file" class="petsImage" id="petsImage1" name="petsImage1"/>
						  ...
						  <div id="end">
								 <label>权限</label>
								  <select name="potype" id="potype" >
										<option value="publicSenderPosts">公开</option>
										<option value="secretSenderPosts">仅好友可见</option>
								  </select> 
