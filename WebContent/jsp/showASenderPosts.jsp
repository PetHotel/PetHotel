...
<form id="formInner" name="formInner" method="post" action="/PetHotel/ApplyServlet">
	  <table  width="779" style="margin-left:105px;"><!-- height="558" -->
                    <tr height="50" align="center">
                      <td colspan="2" align="center" ><strong style="font-size:20px;"> &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;   求收养帖子内容</strong><br/></td>
                    </tr>
                    <tr height="30">
                      <td colspan="2" width="70">  *发帖人：<a href="/PetHotel/ShowOtherUserServlet?uid=${ requestScope.users.uid}">${ requestScope.users.uname}</a>
                    ...
                    <tr height="20">
                      <td colspan="2">*宠物类型：${ requestScope.posts.pettype }</td>
                     
                    </tr>
                    <tr height="20">
                    	<td>*估计开始时间：${requestScope.posts.pobegintime }&nbsp;&nbsp;
                    ...
                    <tr  height="20">
                     <c:if test="${empty requestScope.imgCount }">
							 <td colspan="2">*宠物相关图片（共 0 张图片）</td>
                      </c:if>
                     <c:if test="${!empty requestScope.imgCount }"> 
                            <td colspan="2">*宠物相关图片（共 ${requestScope.imgCount} 张图片）</td>
                     </c:if>
                    </tr>
                    <tr>
                      <td colspan="2">
                    ...
                    <tr>
                      <td><a href="/PetHotel/FindAllPetsByUidServlet?uid=${requestScope.posts.uid}&choice=1" target="_blank">宠物更多详情</a></td>
                      <td>&nbsp;</td>
                    </tr>
                    <tr>
                      <td colspan="2"  height="50"><label>
                        ...
                        <c:if test="${requestScope.posts.uid != sessionScope.user.uid }">
                      		<input type="submit" name="submit" id="submit" value="申请收养"/>
                         </c:if>
                      </label></td>
                    </tr>
                  </table>	               
...
%>
      <div id="mreply" name="mreply"  style="width:710px; height:150px;margin-left:105px;">
...
<%            
                ...
                lstReply = postDao.selectByRetypeId(post.getPoid());
	  			...
	  			%>  
	  			    <div   style="float:right;width:50px;height:26px;margin-top:6px;margin-left:105px;">
      					<a href="javascript:showReply('reply<%=t.getPoid() %>')">回帖</a>
      				</div>
      				<div style=" width:710px; min-height:32px; line-height:30px;margin-left:105px; " >
         				<%=t.getPotime()%>&nbsp; &nbsp;&nbsp;&nbsp;<a href=""><%=tuname %></a>
         					<%=t.getPocontext() %>
      				</div>
      				
		      	     <!-- 信息回复部分 -->
		      	  <div id="reply<%=t.getPoid() %>" align="center" name="reply" style="width:710px; height:150px;margin-left:105px; display:none;">
